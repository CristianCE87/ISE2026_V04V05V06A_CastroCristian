...
// Duración del bit "0" y "1" en ticks del timer
...
/* -------------------------------------------------
   Envía un color sólido a un anillo WS2812B
   ...
   ------------------------------------------------- */
static void rgb_update_ring(uint8_t ring, rgb_color_t color){
//    (void)ring; // Evita warning de parámetro no usado por ahora
    GPIO_TypeDef *port;
    uint16_t pin;
    ...
    // 7. Esperar fin del último pulso PWM
...
//    //    - Canal: CH1
//    //    - Buffer: ws_buffer
//    //    - Longitud: nº de bits totales (8 LEDs × 24 bits)
...
//////    while (hdma_tim4_ch1.State != HAL_DMA_STATE_READY)
//////    {
//////        // Aquí podrías hacer osThreadYield() si quieres
//////        // que el hilo no bloquee tanto.
//////    }
//  // 3. Esperar a que el DMA termine
//    while (hdma_tim4_ch1.State != HAL_DMA_STATE_READY);

//    // 3.1 Esperar a que el timer termine el último ciclo PWM
//    while (__HAL_TIM_GET_FLAG(&htim4, TIM_FLAG_UPDATE) == RESET);
...
/* ============================================================
   FUNCIÓN: Seleccionar el GPIO según ring_id
   ============================================================ */
static void select_gpio_for_ring(uint8_t ring, GPIO_TypeDef **port, uint16_t *pin){
    /*
       Esta función asigna dinámicamente el pin GPIO que corresponde
       al anillo solicitado. Es útil porque el hilo recibe un ring_id
       y necesita saber qué pin debe activar.
    */
...
static void ws2812_fill_buffer(rgb_color_t color){
    ...
    // Índice dentro del buffer global ws_buffer[]
    uint16_t idx = 0;
    ...
    for (int led = 0; led < LEDS_PER_RING; led++)
    {
        ...
        for (int byte = 0; byte < 3; byte++)
        {
            ...
            {
                // Si el bit está a 1 -> pulso largo (T1H)
                if (grb[byte] & (1 << bit))
                {
                    ws_buffer[idx++] = WS2812_T1H;   //bit 1
                }
                // Si el bit está a 0 -> pulso corto (T0H)
                else
                {
